...
Eigen::Matrix3d EulerAngle2Matrix(const Eigen::Vector3d& m) { return util::EulerAngle2Quater(m).matrix(); }

Eigen::Quaterniond EulerAngle2Quater(const Eigen::Vector3d& euler) {  // Convert Euler angles to quarternion
    Eigen::Quaterniond q;
    q = Eigen::AngleAxisd(euler.x(), Eigen::Vector3d::UnitX()) * Eigen::AngleAxisd(euler.y(), Eigen::Vector3d::UnitY()) *
